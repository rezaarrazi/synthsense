...
export const ResultsDashboard = ({ 
  ...
  const loadGuestResults = () => {
    try {
      ...
      setExperiment({
        id: 'guest-simulation',
        idea_text: guestData.idea_text,
        recommended_next_step: guestData.recommended_next_step,
  ...
  const fetchExperimentData = async () => {
    try {
      ...
      // Fetch survey responses with persona data
      ...
      // Transform responses into personas
      const transformedPersonas: Persona[] = responses.map((r: any) => {
        const personaData = r.personas.persona_data;
        const personaName = r.personas.persona_name || 'Anonymous';
    ...
    <div className="flex-1 overflow-auto">
      <div className="max-w-5xl mx-auto p-4 sm:p-6 md:p-8 space-y-4 sm:space-y-6">
        ...
        <div className="bg-card/30 backdrop-blur-sm border border-border/50 rounded-xl p-4 sm:p-6">
          <div className="text-xs sm:text-sm text-muted-foreground mb-2">{experiment.idea_text}</div>
